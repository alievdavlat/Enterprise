...
function generateGraphQLSchema(schemas: ContentTypeSchema[]): string {
  ...
  for (const schema of schemas) {
    const typeName = pascalCase(schema.displayName);
    const fields: string[] = [
      "  id: ID",
      "  documentId: String",
      "  createdAt: String",
    ...
    if (schema.kind === "collectionType" || schema.kind === "component" || schema.kind === "dynamiczone") {
      ...
      const singularQuery = schema.singularName !== p ? schema.singularName : `${schema.singularName}ById`;
  ...
  for (const schema of schemas) {
    ...
    if (schema.kind === "collectionType" || schema.kind === "component" || schema.kind === "dynamiczone") {
      queryResolvers[schema.pluralName] = async (
        ...
        return {
          ...result,
          data: result.data.map((row: Record<string, unknown>) => gqlStripPrivateFields(row, attrs)),
        };
      };
      const singularKey =
        schema.singularName !== schema.pluralName
          ? schema.singularName
          : `${schema.singularName}ById`;
      queryResolvers[singularKey] = async (
        _: unknown,
        { id }: { id: string },
      ) => {
        const row = await db.findOne(schema.collectionName, id);
        return row ? gqlStripPrivateFields(row, attrs) : null;
      };
      mutationResolvers[`create${typeName}`] = async (
        _: unknown,
        { data }: { data: Record<string, unknown> },
      ) => {
        const { data: validatedData, errors } = gqlValidateAndApplyDefaults(data, attrs, false);
        ...
        return gqlStripPrivateFields(created, attrs);
      };
      mutationResolvers[`update${typeName}`] = async (
        _: unknown,
        { id, data }: { id: string; data: Record<string, unknown> },
      ) => {
        const { data: validatedData, errors } = gqlValidateAndApplyDefaults(data, attrs, true);
        if (errors.length > 0) throw new Error(`Validation error: ${errors.join("; ")}`);
        for (const [field, config] of Object.entries(attrs)) {
          if (!config.unique) continue;
          const v = validatedData[field];
        ...
        return gqlStripPrivateFields(updated, attrs);
      };
      mutationResolvers[`delete${typeName}`] = async (
        ...
        return gqlStripPrivateFields(deleted, attrs);
      };
    }
  }
