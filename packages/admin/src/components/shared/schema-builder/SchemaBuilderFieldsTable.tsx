...
export function SchemaBuilderFieldsTable({
  ...
  const entries = Object.entries(selectedCt.attributes) as Array<
    [string, ContentTypeAttributeConfig]
  >;

  /**
   * Drag-drop reorder. Object key order is insertion order in modern JS,
   * so rebuilding `attributes` from the reordered entries persists the
   * new field sequence in both REST + GraphQL responses (which iterate
   * via `Object.entries`) and in the form renderer (same pattern).
   */
  const handleDragEnd = async (result: DropResult) => {
    ...
    const reorderedAttributes = Object.fromEntries(next) as Record<
      string,
      ContentTypeAttributeConfig
    >;
    const updatedCt = { ...selectedCt, attributes: reorderedAttributes };

    // Optimistically update the visible order so the field doesn't snap
    // back while we wait for the round-trip.
    ...
    try {
      ...
      // Roll back to server's view of truth.
      refetchContentTypes();
      fetchContentTypes();
    }
  };
  ...
  return (
    <div className="px-8 py-6">
      <div className="bg-card rounded-xl border border-border overflow-hidden shadow-sm">
        <div className="grid grid-cols-[24px_1fr_200px_100px] border-b border-border bg-muted/40 px-5 py-3.5 text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          <span />
          <span>Name</span>
          <span>Type</span>
          ...
          <Droppable droppableId="schema-fields">
            {(droppableProvided) => (
              <div
                ...
                {entries.map(([name, config], rowIndex) => {
                  const fieldDef =
                        ...
                        <div
                          ...
                          className={`${
                            draggableSnapshot.isDragging
                              ? "shadow-2xl ring-2 ring-primary/40 bg-card"
                              : ""
                          } transition-shadow`}>
                          <div
                            className={`grid grid-cols-[24px_1fr_200px_100px] items-center px-5 py-3.5 border-b border-border/50 hover:bg-muted/20 transition-colors group ${
                            ...
                            <div
                              ...
                              className="flex items-center justify-center w-5 -ml-1 text-muted-foreground/40 hover:text-muted-foreground cursor-grab active:cursor-grabbing transition-colors"
                            ...
                            <span className="text-sm text-muted-foreground capitalize">
                              ...
                              {isComponent && config.component ? (
                                <span className="text-xs ml-1 opacity-70">
                                  →{" "}
                                  {compSchema?.displayName ||
                                    String(config.component)}
                          ...
                          {isComponent && isExpanded && compSchema && (
                            <div
                              ...
                              <div className="ml-8 relative">
                                ...
                                {Object.entries(compSchema.attributes || {})
                                  .length > 0 ? (
                                  Object.entries(
                                    compSchema.attributes as Record<
                                      ...
                                      ContentTypeAttributeConfig
                                    >,
                                  ).map(([childName, childConfig], idx) => {
                                    const childFieldDef =
                                      FIELD_TYPES.find(
                                        (f) => f.id === childConfig.type,
                                      ) || FIELD_TYPES[0];
                                    const ChildIcon = childFieldDef.icon;
                                    return (
                                      <div
                                        key={childName}
