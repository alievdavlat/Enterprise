...
export class SchemaRegistry {
    ...
    validate(schema) {
        ...
        if (!schema.kind ||
            ...
            throw new Error('Content type kind must be "collectionType", "singleType", "component", or "dynamiczone"');
        }
    ...
    validateField(name, field) {
        ...
        if (field.type === "relation" && !field.target) {
            throw new Error(`Relation field "${name}" must have a target content type`);
        }
    }
    /**
    ...
    generateRoutes(uid) {
        ...
        if (!schema)
            throw new Error(`Schema "${uid}" not found`);
        const plural = schema.pluralName;
        const routes = [];
        if (schema.kind === "collectionType" || schema.kind === "component" || schema.kind === "dynamiczone") {
            routes.push({ method: "GET", path: `/api/${plural}`, handler: `${uid}.findMany` }, { method: "POST", path: `/api/${plural}`, handler: `${uid}.create` }, {
                method: "GET",
                path: `/api/${plural}/:id`,
                handler: `${uid}.findOne`,
