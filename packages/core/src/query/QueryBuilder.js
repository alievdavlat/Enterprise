...
export class QueryBuilder {
     ...
     */
    buildSqlWhere(filters, tableName = "") {
        const conditions = [];
        const params = [];
        let paramIndex = 1;
        ...
        for (const [field, value] of Object.entries(filters)) {
            if (field === "$and" && Array.isArray(value)) {
                const subConditions = value.map((f) => {
                    const sub = this.buildSqlWhere(f, tableName);
            ...
            if (field === "$not" && typeof value === "object" && value !== null) {
                const sub = this.buildSqlWhere(value, tableName);
                params.push(...sub.params);
    ...
    buildMongoFilter(filters) {
        ...
        for (const [field, value] of Object.entries(filters)) {
            if (field === "$and" && Array.isArray(value)) {
                mongoQuery.$and = value.map((f) => this.buildMongoFilter(f));
                continue;
            }
            if (field === "$or" && Array.isArray(value)) {
                mongoQuery.$or = value.map((f) => this.buildMongoFilter(f));
                continue;
            }
            if (field === "$not" && typeof value === "object") {
                mongoQuery.$nor = [this.buildMongoFilter(value)];
                continue;
            }
            if (typeof value === "object" && value !== null) {
                const ops = value;
                ...
                if ("$lt" in ops)
                    mongoOps.$lt = ops.$lt;
                if ("$lte" in ops)
                    mongoOps.$lte = ops.$lte;
                if ("$gt" in ops)
                    mongoOps.$gt = ops.$gt;
                if ("$gte" in ops)
                    mongoOps.$gte = ops.$gte;
                if ("$in" in ops)
                    mongoOps.$in = ops.$in;
                if ("$notIn" in ops)
                    mongoOps.$nin = ops.$notIn;
                if ("$contains" in ops)
                    mongoOps.$regex = new RegExp(ops.$contains, "i");
                if ("$startsWith" in ops)
                    mongoOps.$regex = new RegExp(`^${ops.$startsWith}`, "i");
                if ("$endsWith" in ops)
                    mongoOps.$regex = new RegExp(`${ops.$endsWith}$`, "i");
                if ("$null" in ops)
                    mongoOps.$exists = !ops.$null;
    ...
    parsePagination(input = {}) {
        ...
        const start = input.start !== undefined ? input.start : (page - 1) * pageSize;
        return {
            page,
            pageSize,
    ...
    buildSqlOrderBy(sort) {
        ...
        const orderParts = sorts.map((s) => `"${s.field}" ${s.direction === "desc" ? "DESC" : "ASC"}`);
