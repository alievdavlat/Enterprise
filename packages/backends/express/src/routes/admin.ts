...
import type { AdminCtx, AdminRouterOptions } from "./admin/shared";
...
export function createAdminRouter(
  ...
  // Shared context for extracted controller modules (see ./admin/*).
  const ctx: AdminCtx = { router, db, schemaRegistry, options: (options ?? {}) as AdminRouterOptions };
  // No-code builder routes (cron / middlewares / custom routes / services /
  // lifecycles / extensions / plugins / migrations) live in their own module.
  registerBuilderRoutes(ctx);
  // Export / import + scheduled-backup config routes.
  registerDataTransferRoutes(ctx);
