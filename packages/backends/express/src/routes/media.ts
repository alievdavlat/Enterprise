...
const FOLDERS_TABLE = "enterprise_media_folders";
...
export function createMediaRouter(db: DatabaseAdapter, uploadConfig?: UploadConfig): Router {
  ...
  router.get("/files", async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (folderPath !== undefined) {
        filters.folderPath = folderPath === "" || folderPath === "null" || folderPath === "__root__"
          ? { $null: true }
          : folderPath;
      }
  ...
  router.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const { name, caption, alternativeText, folderPath } = req.body;
        ...
        if (folderPath !== undefined) payload.folderPath = folderPath === "" || folderPath === null ? null : folderPath;
  ...
  /**
   * POST /api/upload/files/move – Move multiple files to folder or global
   */
  router.post("/files/move", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { ids, folderPath } = req.body;
      if (!Array.isArray(ids) || ids.length === 0) {
        return res.status(400).json({ error: { status: 400, message: "ids array required" } });
      }
      const targetPath = folderPath === "" || folderPath === null || folderPath === undefined ? null : folderPath;
      for (const id of ids) {
        const row = await db.findOne(MEDIA_TABLE, paramId(id));
        if (row) {
          await db.update(MEDIA_TABLE, paramId(id), { folderPath: targetPath });
        }
      }
      res.json({ data: { moved: ids.length, folderPath: targetPath } });
    } catch (err) {
      next(err);
    }
  });
  ...
  /**
   * GET /api/upload/folders – List media folders
   */
  router.get("/folders", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await db.findMany(FOLDERS_TABLE, {
  ...
  /**
   * POST /api/upload/folders – Create folder
   */
  router.post("/folders", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { name, path: folderPath, parentPath, location } = req.body;
      if (!name || typeof name !== "string" || !name.trim()) {
        return res.status(400).json({ error: { status: 400, message: "name required" } });
      }
      const parent = parentPath ?? location ?? null;
      const safeName = name.trim().replace(/[/\\]/g, "");
      const p = folderPath ?? (parent && parent !== "/" ? `${parent.replace(/\/+$/, "")}/${safeName}` : `/${safeName}`);
      const existing = await db.findOneBy(FOLDERS_TABLE, { path: p });
      if (existing) {
        return res.status(409).json({ error: { status: 409, message: "Folder already exists" } });
      }
      const row = await db.create(FOLDERS_TABLE, {
        name: name.trim(),
        path: p,
        parentPath: parent,
      });
      res.status(201).json({
        ...
        parentPath: (row as { parentPath?: string }).parentPath ?? null,
      });
    } catch (err) {
      next(err);
    }
  ...
  /**
   * DELETE /api/upload/folders/:id – Delete folder (media stay, folderPath cleared if matched)
   */
  router.delete("/folders/:id", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const id = paramId(req.params.id);
      const row = await db.findOne(FOLDERS_TABLE, id);
      if (!row) {
        return res.status(404).json({ error: { status: 404, message: "Folder not found" } });
      }
      const folderPath = (row as { path?: string }).path;
      await db.delete(FOLDERS_TABLE, id);
      if (folderPath) {
        const allMedia = await db.findMany(MEDIA_TABLE, {
          pagination: { page: 1, pageSize: 1000 },
          filters: { folderPath },
        });
        for (const m of allMedia.data as Record<string, unknown>[]) {
          const mediaId = m.id as string | number;
          if (mediaId != null) await db.update(MEDIA_TABLE, mediaId, { folderPath: null });
        }
      }
      res.json({ data: { id, deleted: true } });
    } catch (err) {
